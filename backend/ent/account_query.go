...
type AccountQuery struct {
	...
	withPackageChannels *PackageChannelQuery
...
// QueryPackageChannels chains the current query on the "package_channels" edge.
func (_q *AccountQuery) QueryPackageChannels() *PackageChannelQuery {
	query := (&PackageChannelClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *AccountQuery) Clone() *AccountQuery {
	...
	return &AccountQuery{
		...
		withPackageChannels: _q.withPackageChannels.Clone(),
...
// WithPackageChannels tells the query-builder to eager-load the nodes that are connected to
// the "package_channels" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *AccountQuery) WithPackageChannels(opts ...func(*PackageChannelQuery)) *AccountQuery {
	query := (&PackageChannelClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withPackageChannels = query
	return _q
}
...
func (_q *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := _q.withPackageChannels; query != nil {
		if err := _q.loadPackageChannels(ctx, query, nodes,
			func(n *Account) { n.Edges.PackageChannels = []*PackageChannel{} },
			func(n *Account, e *PackageChannel) { n.Edges.PackageChannels = append(n.Edges.PackageChannels, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *AccountQuery) loadPackageChannels(ctx context.Context, query *PackageChannelQuery, nodes []*Account, init func(*Account), assign func(*Account, *PackageChannel)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(packagechannel.FieldAccountID)
	}
	query.Where(predicate.PackageChannel(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(account.PackageChannelsColumn), fks...))
	}))
