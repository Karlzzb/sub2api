...
func (_q *GroupQuery) QueryPackageChannels() *PackageChannelQuery {
...
func (_q *GroupQuery) WithPackageChannels(opts ...func(*PackageChannelQuery)) *GroupQuery {
	query := (&PackageChannelClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := _q.withPackageChannels; query != nil {
		if err := _q.loadPackageChannels(ctx, query, nodes,
			func(n *Group) { n.Edges.PackageChannels = []*PackageChannel{} },
			func(n *Group, e *PackageChannel) { n.Edges.PackageChannels = append(n.Edges.PackageChannels, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *GroupQuery) loadPackageChannels(ctx context.Context, query *PackageChannelQuery, nodes []*Group, init func(*Group), assign func(*Group, *PackageChannel)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Group)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(packagechannel.FieldGroupID)
	}
	query.Where(predicate.PackageChannel(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.PackageChannelsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
